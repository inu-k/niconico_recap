...
func GetSummary(c *gin.Context) {
	...
	if date != "" {
		...
		if startDate == "" {
			st, err = time.Parse("2006-01-02", "1900-01-01")
		} else {
...
func GetDailySummary(c *gin.Context) {
	...
	rows, err := Db.Query("select B.tag, count(*) from history as A inner join video_tag_info as B on A.video_id=B.video_id where A.watch_date>=$1 and A.watch_date<$2 group by B.tag order by count(*) desc",
